    ...
    <div role="group" aria-label={label} className={cn("inline-flex gap-[3px] rounded-[9px] bg-fill p-[3px]", className)}>
      {options.map(({ id, label: optionLabel }) => (
        <button
          ...
          className={cn(
            "min-h-[28px] min-w-0 flex-1 rounded-md px-3 text-[0.8125rem] font-bold whitespace-nowrap transition-colors outline-none focus-visible:outline-2 focus-visible:outline-ring",
            value === id
